...
export default async function PortfolioPage({ params }: PortfolioPageProps) {
  ...
  let profile = {
    ...
    tagline:
      "Building high-performance enterprise web applications and crafting reusable frontend interfaces.",
  ...
  try {
    ...
    experiences =
      dbExperiences.length > 0 ? dbExperiences : getDefaultExperiences();
    projects = dbProjects.length > 0 ? dbProjects : getDefaultProjects();

    if (dbUser) {
      const dbProfile = await userRepo
        .getProfileByUserId(dbUser.id)
        .catch(() => null);
      if (dbProfile) {
        profile = {
          name: dbProfile.name,
          ...
          linkedinUrl:
            dbProfile.linkedinUrl || "https://www.linkedin.com/in/namkndev86/",
          twitterUrl: dbProfile.twitterUrl || "https://x.com/namkndev86",
          resumeUrl: dbProfile.resumeUrl || "/documents/resume.pdf",
    ...
    console.warn(
      "⚠️ Database connection failed. Rendering portfolio in static mode with mock data.",
      err,
    );
  ...
  return (
    <LocaleProvider locale={locale as Locale} dictionary={dictionary}>
      <div className="w-full flex flex-col items-center">
        ...
        <section
          id="about"
          className="w-full py-16 md:py-28 border-b border-card-border/40 relative"
        >
          ...
          <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-7 flex flex-col gap-6">
              <span className="font-mono text-xs text-primary font-semibold tracking-widest uppercase">
                {t("portfolio.hero.overview")}
              </span>
              <h1 className="font-display font-bold text-4xl md:text-6xl text-foreground leading-tight">
                {locale === "vi" ? (
                  <>
                    Thiết kế các hệ thống mở rộng một cách{" "}
                    <span className="text-primary">toàn vẹn</span>.
                  </>
                ) : locale === "ja" ? (
                  <>
                    整合性と拡張性を両立したシステム{" "}
                  ...
                  <>
                    Designing platforms that{" "}
                    <span className="text-primary">scale</span> with integrity.
                  </>
                )}
              </h1>
        ...
        <section
          id="projects"
          className="w-full py-20 border-b border-card-border/40 relative"
        >
                  ...
                  <div className="p-6 md:p-8 flex flex-col flex-1 gap-4">
                    ...
                    <div className="flex flex-wrap gap-2 py-2">
                      {project.techStack.map((tech: string) => (
                        <span
                          key={tech}
                          className="px-2.5 py-1 rounded bg-card-border/60 border border-card-border/80 text-[10px] font-mono text-gray-300 uppercase"
                        >
                          {tech}
                        </span>
                      ))}
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2 font-mono text-sm text-gray-400">
                <p>
                  ...
                  <span className="text-white">namkndev86@gmail.com</span>
                </p>
                <p>
                  {t("portfolio.contact.timezone")}:{" "}
                  <span className="text-white">UTC+07:00 / Hanoi, Vietnam</span>
                </p>
              </div>
            </div>
...
function getDefaultExperiences(): any[] {
  return [
    ...
    },
    {
      ...
      location: "Hanoi, Vietnam",
      description:
        "Developed document workflows and approval systems using React, MobX, and Ant Design. Extended and maintained CKEditor 5/Tiptap Editor integrations. Built RESTful APIs using Spring Boot, Express, and Prisma ORM.",
    },
    {
      id: "4",
      ...
      location: "Hanoi, Vietnam",
      description:
        "Developed note management features (CRUD) using Next.js 13 and Ant Design. Implemented App Router, dynamic routing, and built responsive user interfaces using TailwindCSS.",
    },
  ];
}
